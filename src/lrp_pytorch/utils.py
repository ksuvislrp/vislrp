...
def get_activations(A,X,L,layers):
    for l in range(L):        
        if cuda:
            A[l+1] = layers[l].cuda().forward(A[l].cuda()).cpu()
        else:
...
def lrp_R_backPropagation_step(A, R, l, mean, std, methodObj, layers, contrastive_signal, **kwargs):
    ...
    if cuda:
        ...
        # get activation
        A[l] = (A[l].data).requires_grad_(True).cuda()

        if isinstance(layers[l], torch.nn.MaxPool2d):
            layers[l] = torch.nn.AvgPool2d(2).cuda()
        ...
        if isinstance(layers[l], torch.nn.Conv2d) or isinstance(layers[l], torch.nn.AvgPool2d):
            ...
            if isinstance(layers[l], torch.nn.AvgPool2d):
                ...
                const_weight = rms(layers[l].weight)  
            ...
            # activation control
            I_with_Al_shape = rms(A[l]).cuda()
            new_Al =  (theta0*I_with_Al_shape+ theta1*A[l].cuda()).float().clone().detach().requires_grad_(True)

            if 'lrp-z_beta' == method:
                pass
            else:
                z = incr(utils.newlayer(layers[l].cuda(), rho).forward(new_Al))  
                s = (R[l+1].cuda()/z)
                (z*s.data ).sum().backward(); c = new_Al.grad
                R[l] = (new_Al * (alpha * c)).data.cpu()


                if beta > 0:
                    new_Al_beta = new_Al.clone().detach().requires_grad_(True)
                    z_beta = incr(utils.newlayer(layers[l].cuda(),  rho_beta).forward(new_Al_beta))                
                    s_beta =  (R[l+1].cuda()/z_beta) 
                    (z_beta * s_beta.data ).sum().backward(); c_beta = new_Al_beta.grad

                    R[l] = (new_Al * (alpha * c- beta* c_beta)).data.cpu()

            R[l][torch.isnan(R[l])] = 0

            new_Weight = layers[l].weight.requires_grad_(False).detach().cpu().numpy() if isinstance(layers[l], torch.nn.Conv2d) else "None"        
    ...
    # No Cuda
        ...
        if isinstance(layers[l], torch.nn.Conv2d) or isinstance(layers[l], torch.nn.AvgPool2d):
            ...
            if 'lrp-z_beta' == method:
                ...
                z = incr(utils.newlayer(layers[l], rho).forward(new_Al))  
                s = (R[l+1]/z)
                (z*s.data ).sum().backward(); c = new_Al.grad
...
def lrp_backPropagation_last(A,R,mean,std, layers):
    if cuda:
        A[0] = (A[0].data).requires_grad_(True).cuda()
        ...
        R[0] = (A[0]*c+lb*cp+hb*cm).data.cpu()       
